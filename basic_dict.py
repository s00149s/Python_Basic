def define_dict():
    ...
    dct['volleyball'] = 6   #  키와 값 추가

    print(dct)
    # 길이 확인 가능, 포함여부 확인 가능 -> 대상은 키 목록을 대상으로 한다
    # 연결, 반복 지원 X
    print("LENGTH of dct : ", len(dct)) # (키:값)의 갯수 확인 가능
    ...
    print("(키, 값) 쌍튜플의 목록 : ", dct.items(), type(dct.items()))  # type -> dict_items 쌍튜플 타입

    # 응용
    # 포함 여부는 key를 대상으로 한다
    # 사전의 값의 포함 여부를 확인?
    # dct 값 중에서 9 값이 포함되어 있는가?
    ...
    # 키의 목록, 값의 목록이 따로 있을 때 zip 객체를 이용하여 합치기
    keys = ('one', 'two', 'three')
    values = (1, 2, 3, 4)
    d3 = dict(zip(keys, values))    # zip 객체는 여러개의 순차객체를 쌍으로 묶어줌
    print(d3, type(d3))
...
def dict_methods():
    ...
    print(lst_keys, type(lst_keys)) # dict_keys는 특수한 순차형이기 때문에 list형으로 변환 가능

    # 사전의 값의 목록 : values()
    values = dct.values()
    print("값 목록 : ", type(values))  # dict_values
