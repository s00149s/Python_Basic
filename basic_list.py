...
def list_oper():
    ...
    print(copy)
    print(copy is lst)  # False : 슬라이싱 -> 새 객체가 생성(별개의 객체로 판단)

    # 연결(+) : 원본을 변경하지 않고 단순히 두 list를 연결한 새 리스트를 반환
    #   vs extend : 원본 뒤에 다른 리스트를 연장 -> 내부 데이터 변경
    print(lst + ["Java", True, 3.14159])
    print("원본:", lst)

    # append vs extend
    copy.append(["Java", True, 3.14159])    # 개별 요소(리스트)의 추가
    print(copy)
    copy.extend(["Java", True, 3.14159])    # 배열 자체를 확장시켜 원래 리스트 안에 다른 리스트를 연결하여 확장
    print("Extend : ", copy)
    ...
    # 포함여부 확인 : in, not in
    print("Python" in lst) # lst에 "Python"이 포함?
...
def stack_ex():
    ...
    if len(stack):  # 스택이 비어있지 않으면
        print("POP : ", stack.pop())    # 빈 리스트로부터 pop은 에러
    else:
        print("스택이 비어있습니다.")
...
def loop():
    ...
    # 순차 자료형은 for ~ in 문으로 차례대로 요소를 전달 받을 수 있다(별도 인덱스 변수는 X)
    for word in words:
        print("WORD : ", word)
    else:
        print()
