...
def write01():
    ...
    write_size = f.write("Life si too short, you need Python")
    print(write_size)
    f.close();
...
def pickle_dump():
    with open("./sample/players.bin", "wb") as f:   # 자원 자동 정리, pickle 사용을 위해서 모드를 binary로
        data = {"baseball": 9}
        pickle.dump(data, f)
    print("덤프 완료!")
...
def pickle_dump_multi():
    ...
    with open("./sample/players.bin", "wb") as f:
        pickle.dump({"baseball" : 9}, f, 1)    # 프로토콜 버전 명시 가능
