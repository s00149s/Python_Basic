def define_str():   # 문자열의 정의
    ...
    # 여러 줄 주석을 사용할 경우에도 사용 가능
    # 메서드 정의부 바로 아래 여러 줄 문자열을 입력하면
    # help 명령어를 이요ㅗㅇ해서 해당 메서드의 주석을 볼 수 있다
    # -> docstring


def string_oper():  # 문자열의 연산
    """
    문자열 연산 연습
        str : len -> 길이 확인
            연결(+), 반복(*), 포함 여부 확인 가능
    ...
    # 슬라이싱 [시작경계:끝경계:간격(step)]
    print("[::2] 슬라이싱", str1[::2])  # 처음부터 끝까지, 2간격으로
    print("[::2] 슬라이싱", str1[::-1]) # 간격이 음수면 방향이 반대
    ...
    # 포함 여부 확인 : in, not in
    print("P" in str1)
    print("P" not in str1)  # not ("P" in str1)



def transfrom_methods():
    """
    대소문자 변환 관련 메서드들
    """
    s = "i like Python"
    ...
    print("원본 : ", s)   # str 객체는 immutable -> 원본 변경되지는 않음
...
def search_methods():
    ...
    if "Like" in s[21:]: print("3rd Index", s.index("Like", 21))    # 에러 회피 가능
...
def modify_replace_methods():
    ...
    # 기본적으로 Strip은 공백 문자를 제거 -> 임의의 문자열을 제거 가능
    s = "---------Alice and the Heart Queen----------"
...
def align_methods():
    ...
    print("CENTER:[", s.center(60), "]", sep="")    # 60 자리 확보 중앙정렬
    ...
    print("ZFILL:", "1234567890".zfill(5))  # 확보한 자리는 최소 공간, 자릿수가 넘쳐도 내용은 잘리지 않는다
...
def string_format():
    ...
    print(fmt %(10, '사과', 3))
    print("현재 이자율은 %.2f%%입니다" % 1.234567) # %f 포멧은 부가적으로 소숫점 자리를 제한할 수 있다.
