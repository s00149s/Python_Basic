...
# 인수명 앞에 *를 붙여 선언
def get_total(*args):   # 여러 개의 인수를 전달 -> 합산 후 밥ㄴ환
    total = 0
    print("인수 : ", args, type(args))
    for x in args:
        total += x
    return total
print(get_total(1, 3, 5, 7, 9)) # 튜플 객체 args로 전달
...
print("(1, 2, 3, 4, 5) : ", get_total2(1, 2, 3, 4, 5))
print("(1, 2, '3', 4, 5) : ",get_total2(1, 2, "3", 4, 5))
print("(1, 2, [3, 4, 5]) : ", get_total2(1, 2, [3, 4, 5]))
...
# 순서 중요 : 고정 인수들 -> 가변인수 -> 사전 키워드 인수 순
args_test(10, 20, 30, 40, 50, option1="value1", option2="value2")

# 함수도 객체다
# 함수도 변수에 참조 할당 되거나, 다른 함수의 인수로 전달될 수 있음
def plus(a, b):
    return a + b
def minus(a, b):
    return a - b
def calculator(a, b, func): # a, b -> 숫자, func -> 함수 전달
    if (callable(func)):    # 넘어온 인수 func가 실행 가능 객체(함수) 인가?
        return func(a, b)
...
def clean_string(strings, *funcs):
    ...
    for string in strings:
        # print(string)
        for func in funcs:
            if callable(func):  #넘어온 인자가 실행 가능 객체임 확인
                string = func(string)
