...
def ref_count():
    ...
    y = x   # y에 x의 참조 주소를 복사 -> 두 개의 주소값은 같아지고 참조카운트가 하나 늘어남
    print("REFCOUNT : ", sys.getrefcount(x))
...
def object_id():
    ...
    # 가변 바료형 : 값이 같아도 별개의 객체
    lst1 = [1, 2, 3]
    lst2 = [1, 2, 3]
    print("list : ", hex(id(i1)), hex(id(i2)))

    lst3 = lst2 # 참조 복사 : 객체 주소의 복사
    print("참조 복사 : ", hex(id(lst2)), hex(id(lst3)))

    # == 은 동등성의 비교(값이 같으냐), is 동일성의 비교(같은 객체냐)의 비교
